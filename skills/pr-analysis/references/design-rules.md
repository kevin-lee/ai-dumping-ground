...
- Syntax colors come from six tokens (`--sy-comment`, `--sy-keyword`, `--sy-string`, `--sy-number`, `--sy-name`, `--sy-type`) defined for light and dark and checked at 4.5:1 or more on the diff well and on every row tint, so they need no CVD copy. Types and function names get different colors. Fragments never use them. A theme carries its own six, and `check.mjs` verifies them against the theme's well, its row tints, and the CVD row tints.
